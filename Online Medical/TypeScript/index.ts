...
async function EditMedicine(id:string)
{
    try
    {
...
async function medicinedetails()
{
    try
    {
        ...
        if(medicines.length>0)
        {
            ...
            {
                idCount=Number(medicine.medicineID.split('').splice(3,2).join(''));
...
async function DeleteMedicines(id:string):Promise<void>
{
...
async function UpdateMedicines(id:string,medicine:Medicines):Promise<void>
{
    const response=await fetch(`http://localhost:5248/api/medicine/${id}`,{
        method:'PUT',
        headers: {
...
async function UpdateOrders(id:string,order:Order):Promise<void>
{
